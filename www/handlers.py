...
import re, time, json, logging, hashlib, base64, markdown2
import asyncio
...
@get('/manage/comments')
async def manage_comments(*, page='1', request):
    page_index = get_page_index(page)
...
async def autnenticate(*, email, passwd):
    if not email:
...
async def api_register_user(*, email, name, passwd):
    ...
    user = User(id=uid, name=name.strip(), email=email, passwd=password, image=image)
    await user.save()
